          ...
          >
            {Icon && (
              typeof Icon === 'string' ? (
                <img 
                  src={Icon} 
                  className={`blob-icon custom-svg-icon ${['#FFFFFF', '#fdfdf5', '#ffef00'].includes(color) ? 'white-blob-icon' : ''}`} 
                  style={{ 
                ...
                <Icon className={`blob-icon ${['#FFFFFF', '#fdfdf5', '#ffef00'].includes(color) ? 'white-blob-icon' : ''}`} size={size * 0.18} />
              )
            )}
            <span className={`blob-label ${['#FFFFFF', '#fdfdf5', '#ffef00'].includes(color) ? 'white-blob-text' : ''}`}>{label}</span>
          </div>
        </motion.button>
      </motion.div>
    </motion.div>
...
const BlobHero: React.FC<BlobHeroProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    ...
    const blobSizeScale = isMobile ? 0.52 : (isLaptop ? 0.82 : (isMedium ? 1.0 : 1.0)); 

    // Elliptical Logic for Laptop & Medium - Refined for more air
    const ySquash = isLaptop ? 0.75 : (isMedium ? 1.0 : 1.0);
    const xScale = isLaptop ? 1.45 : (isMedium ? 1.15 : 1.0);
    const stretchX = isLaptop ? 1.35 : (isMedium ? 1.05 : 1.0);
    ...
    const blobColors = [
      ...
      '#ffef00', // Yellow
      '#00811f', // Green
      '#132c62', // Specific Requested Blue
      '#760089', // Purple
    ];
