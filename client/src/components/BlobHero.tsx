...
const BlobHero: React.FC<BlobHeroProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    ...
    const blobSizeScale = isMobile ? 0.52 : (isLaptop ? 0.82 : (isMedium ? 1.05 : 1.0)); 

    // Elliptical Logic for Laptop & Medium - Refined for more air
    const ySquash = isLaptop ? 0.75 : (isMedium ? 1.25 : 1.0);
    const xScale = isLaptop ? 1.45 : (isMedium ? 1.1 : 1.0);
    const stretchX = isLaptop ? 1.35 : (isMedium ? 1.0 : 1.0);
